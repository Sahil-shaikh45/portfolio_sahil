    ...
    <section id="projects" className="py-24 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          >
            Featured <span className="text-accent-teal dark:text-accent-glowTeal glow-text-teal">Projects</span>
          </motion.h2>
          <motion.div
            initial={{ width: 0 }}
        ...
        <div className="flex justify-center space-x-3 mb-12">
          {categories.map((c) => (
            <button
              ...
              className={`px-6 py-2.5 rounded-2xl text-sm font-semibold transition-all duration-200 border backdrop-blur-md ${
                filter === c.id
                  ? 'bg-accent-teal text-white border-accent-teal shadow-lg shadow-accent-teal/20 scale-105'
                  : 'bg-white/70 dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:border-accent-teal/40'
              }`}
            >
              {c.label}
            </button>
        ...
        {/* Projects Grid with 3D Scroll Parallax */}
        <Card3DScroll rotateXAmount={16} translateZAmount={50}>
          <motion.div 
            layout
                  ...
                  >
                    <Card3DTilt maxTilt={10} scale={1.02}>
                      <div
                        ...
                        className={`cursor-pointer rounded-3xl border text-left glass-panel-3d transition-all duration-300 overflow-hidden group ${
                          isExpanded 
                            ? 'border-accent-teal/80 dark:border-accent-teal/70 shadow-2xl' 
                            : 'border-slate-200/60 dark:border-slate-800/60 hover:border-accent-teal/50 dark:hover:border-accent-teal/40'
                        }`}
                      >
                        <div className="p-8">
                          {/* Header */}
                          <div className="flex justify-between items-start mb-4">
                            <span className="px-3.5 py-1 rounded-full text-xs font-mono font-bold uppercase tracking-wider bg-accent-teal/15 text-accent-teal dark:text-accent-glowTeal border border-accent-teal/30">
                            ...
                            <div className="flex items-center space-x-3 text-slate-400 dark:text-slate-500">
                              <a
                                ...
                                className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:text-accent-teal dark:hover:text-accent-glowTeal hover:scale-110 transition-all duration-200"
                              >
                          ...
                          {/* Title */}
                          <h3 className="font-display font-bold text-xl sm:text-2xl text-slate-900 dark:text-white mb-3 group-hover:text-accent-teal dark:group-hover:text-accent-glowTeal transition-colors duration-200 flex items-center justify-between">
                          ...
                          <div className="flex flex-wrap gap-2 mb-6">
                            {project.tech.map((t) => (
                              <span 
                                key={t}
                                className="px-3 py-1 rounded-xl text-xs font-mono font-medium bg-slate-100 dark:bg-slate-800/80 text-slate-700 dark:text-slate-300 border border-slate-200/50 dark:border-slate-700/50"
                              >
                                {t}
                              </span>
                            ))}
                          ...
                          {/* Toggle button */}
                          <div className="flex items-center text-xs font-mono font-bold text-accent-teal dark:text-accent-glowTeal uppercase tracking-wider">
                            {isExpanded ? (
                              <>
                                <span>Show Less</span>
                          ...
                          <AnimatePresence>
                            {isExpanded && (
                              ...
                              >
                                <h4 className="text-xs font-mono font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2 flex items-center">
                                  <Code className="w-4 h-4 mr-1.5 text-accent-teal" />
                                  Implementation Details
                                </h4>
                                <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed whitespace-pre-line font-normal">
                                  {project.details}
                                </p>
                              </motion.div>
                            )}
